...
\]
This gradient is a function $\nabla f:\mathbb{R}^n\rightarrow\mathbb{R}^n$, so it can be thought of as a vector field in $\mathbb{R}^n$, which we call the \emph{gradient field} of the function $f$.

\section*{Gradient Fields}

Let's look at some examples of gradient fields.
...
\nabla f(x,y) = (2x, 2y).
\]
We graph this gradient field below.

PICTURE
\end{example}

\begin{example}
Consider the function $f(x,y) = xy$. The gradient of this function is
\[
\nabla f(x,y) = (y,x).
\]
We graph this gradient field below.
...
Taking a slightly different perspective, we can start with a vector field $\vec{F}$, and determine whether we can find a function $f$ such that $\vec{F}=\nabla\vec{f}$, so that $\vec{F}$ is the gradient field of $f$. If this is possible, we say that $\vec{F}$ is a conservative vector field.

\begin{definition}
A vector field $\vec{F}:\mathbb{R}^n\rightarrow\mathbb{R}^n$ is \emph{conservative} if there is some function $f:\mathbb{R}^n\rightarrow\mathbb{R}$ such that $\nabla f = \vec{F}$. Then $f$ is called a \emph{potential function} for $\mathbf{F}$.
\end{definition}
%
%\begin{example}
%Consider the vector field $\vec{F}(x,y) = (x+y, x+y^2)$. We would like to find a function $f:\mathbb{R}^2\rightarrow{R}$ such that $\nabla f = \vec{F}$.
%
%Notice that if we integrate the first coordinate of $F$ with respect to $x$, we have
%\[
%\int (x+y)\;dx = \frac{1}{2}x^2 + yx + (\text{terms which are constant with respect to $x$}).
%\]
%Integrating the second coordinate, $x+y^2$, with respect to $y$, we have 
%\[
%\int(x+y^2)\;dy = xy + \frac{1}{3}y^3 + (\text{terms which are constant with respect to $y$}).
%\]
%Piecing these together, we see that if we take $f(x,y) = \frac{1}{2}x^2+xy + \frac{1}{3}{y^3}$, then $\vec{F}$ is the gradient of $f$. Thus $\vec{F}$ is a conservative vector field.
%
%Of course, this isn't the only possible choice for $f$. We could also have chosen $f(x,y) = \frac{1}{2}x^2+xy + \frac{1}{3}{y^3} + 1$, or $f(x,y) = \frac{1}{2}x^2+xy + \frac{1}{3}{y^3} - \pi$, and so on.
%\end{example}
%
%\begin{example}
%Consider the vector field $\vec{F}(x,y) = (0,x)$. We will show that $F$ is not a conservative vector field.
%
%Suppose there were a function $f(x,y)$ such that $\nabla f(x,y) = (0,x)$. Then $\frac{\partial f}{\partial x} = 0$, so $f$ is constant with respect to $x$. However, $\frac{\partial f}{\partial y} = x$, so $f$ has the form
%\[
%f(x,y) = xy + (\text{terms which are constant with respect to $y$}).
%\]
%That is, we can write $f(x,y)$ in the form $f(x,y) = xy+ g(x)$ for some function $g$. Then,
%\[
%\frac{\partial f}{\partial x} = y + g'(x),
%\]
%which can't be identically $0$.
%
%Thus, there is no function $f$ such that $\nabla f = \vec{F}$, so $\vec{F}$ is not a conservative vector field.
%\end{example}
%
%Conservative vector fields are very important in mathematics, and we will later return to the question of when a vector field is conservative.


So, one way to show that a vector field $\mathbf{F}$ is conservative is by finding a potential function $f$.
...
We've seen that if a vector field is conservative, then its derivative matrix is symmetric. But is the converse true? That is, if the derivative matrix is symmetric, does that mean that the vector field is conservative? We'll come back to this question later.
